  ...
  async function load(){
    ...
    try {
      ...
      if (!slug) {
        ...
        const [bRes, carsRes] = await Promise.all([getBrandBySlug(slug), getCars()])
        if (!mountedRef.current) return
        setBrand(bRes)
        setCars(carsRes)
      }
  ...
  return (
    <main className="container section-space-large" aria-label={brand.name}>
      <div className="brand-back">
        <Link to="/brands" className="meta motion-link">← VOLTAR PARA MARCAS</Link>
      </div>
      ...
      <header className="brand-detail-hero">
        ...
        {brand.description ? <p className="muted" style={{ marginTop: 8 }}>{brand.description}</p> : null}
          <div style={{ marginTop: 12, display: 'flex', gap: 12, alignItems: 'center' }}>
           <div className="meta">{brandCars.length} CARROS</div>
         </div>
        {brandCars.length > 0 && (
          <div style={{ marginTop: 18 }} className="brand-hero-media card card-media">
            {/* show image of first car as cinematic crop */}
            {brandCars[0].image ? <img src={brandCars[0].image} alt={`${brand.name} hero`} /> : null}
          </div>
        )}
      </header>
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 18, alignItems: 'center' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div className="card-title" style={{ fontSize: '2rem', color: 'black' }}>{brandCars.length}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Featured machine */}
      {featured && (
        <section className="brand-detail-feature" style={{ marginTop: 28 }} aria-label="Featured machine">
          <div className="card">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 420px', gap: 20 }}>
              <div className="card-body--lg">
                <div className="card-meta">CARRO EM DESTAQUE</div>
                <h2 style={{ color: 'black' }} className="card-title">{featured.name}</h2>
                <div className="muted" style={{ marginTop: 6 }}>{featured.brand} — {featured.year}</div>
                {featured.description ? <div className="muted card-description" style={{ marginTop: 12 }}>{featured.description}</div> : null}
                <div style={{ marginTop: 14 }}>
                  <Link to={`/cars/${featured.slug}`} className="garage-cta motion-link" style={{ color: 'black' }}>VER CARRO →</Link>
                </div>
              </div>
      ...
      <section className="brand-machines" style={{ marginTop: 32 }} aria-label="Machines from the archive">
        <h3 className="display">CARROS DO ARQUIVO</h3>
        {brandCars.length === 0 ? (
          <div style={{ marginTop: 12 }} className="muted">NÃO HÁ CARROS NO ARQUIVO ATUAL.</div>
        ) : (
                ...
                <Link key={car.id} to={`/cars/${car.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <article className="card" aria-label={`${car.brand} ${car.name}`}>
                    ...
                    <div className="card-body">
                      ...
                      {car.description ? <div className="muted card-description">{car.description}</div> : null}
                    </div>
                  </article>
                </Link>
              ))}
      ...
      {brandCars.length > 1 && (
        ...
        <h3 className="display">ÍNDICE DO ARQUIVO</h3>
          <div style={{ marginTop: 12, display: 'grid', gap: 8 }}>
            {brandCars.map((c, i) => (
              <Link key={`idx-${c.id}`} to={`/cars/${c.slug}`} className="motion-link" style={{ textDecoration: 'none', color: 'inherit' }}>
                <div className="brand-machine-index" style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--color-border)' }}>
                  <div style={{ fontWeight: 700 }}>{String(i+1).padStart(2,'0')} — {c.name}</div>
      ...
      {/* Editorial break */}
      <section style={{ marginTop: 32, textAlign: 'center' }} className="brand-editorial-break">
